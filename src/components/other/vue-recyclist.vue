<template>
  <div class="vue-recyclist">
    <div ref="list" class="vue-recyclist-items" :style="{height: height + 'px'}">
      <div v-for="(item, index) in items" class="vue-recyclist-item" :style="{transform: 'translate3d(0,' + item.top + 'px,0)'}" :key="index">
      ...
      <div class="vue-recyclist-pool">
        ...
        <div :ref="'item'+index" v-for="(item, index) in items" v-if="!item.tomb && !item.height"
          class="vue-recyclist-item vue-recyclist-invisible" :key="index">
    ...
    <div v-show="spinner && !nomore">
      <slot name="spinner">
        <div class="spinner">ヽ（≧□≦）ノ 正在加载 ヽ（≧□≦）ノ</div>
      </slot>
    </div>

    <div v-show="nomore">
      <slot name="nomore">
        <div class="nomore">-- (●'◡'●) 我是有底线的 (●'◡'●) --</div>
      </slot>
    </div>
  </div>
</template>
...
<style lang="scss" scoped>
  ...
  .vue-recyclist::-webkit-scrollbar {display:none}
  .vue-recyclist {
    overflow-x: hidden;
    overflow-y: scroll;
